...
function generate_from_IR_julia(IR, IRtypes::Union{IR_entry_types, Nothing} = nothing)
    if IRtypes === nothing
    ...
    elseif node_type == IR_loop_node
        ...
        if IR.type == IRtypes.while_loop
            ...
            code *= generate_from_IR_julia(IR.body, IRtypes);
            code *= "end\n";
    ...
    elseif node_type == IR_conditional_node
        code = "if " * generate_from_IR_julia(IR.condition, IRtypes) * "\n";
        code *= generate_from_IR_julia(IR.body, IRtypes);
        if !(IR.elsepart===nothing)
            code *= "else\n" * generate_from_IR_julia(IR.elsepart, IRtypes);
        end
        code *= "end\n";
...
function generate_named_op(IR::IR_operation_node, IRtypes::Union{IR_entry_types, Nothing} = nothing)
    ...
    elseif op === :INIT_MATRIX_IJ_FV
        ...
        code = "set_matrix_indices!(" * generate_from_IR_julia(IR.args[2], IRtypes) * ", " * 
    ...
    elseif op === :GHOST_EXCHANGE_FV
        if length(IR.args) < 2
            ...
            code *= "exchange_ghosts_fv(var, mesh, dofs_per_node, "*generate_from_IR_julia(IR.args[2], IRtypes)*", config);"
        end
        
    elseif op === :GLOBAL_SOLVE
        # not in-place
        # code =  generate_from_IR_julia(IR.args[2], IRtypes) * " .= linear_system_solve("* generate_from_IR_julia(IR.args[3], IRtypes) *", "* generate_from_IR_julia(IR.args[4], IRtypes) *", config);"
        # in-place
        code = "linear_system_solve!("* generate_from_IR_julia(IR.args[3], IRtypes) *", "* generate_from_IR_julia(IR.args[4], IRtypes) *", "* generate_from_IR_julia(IR.args[2], IRtypes) * ", config);"
        
    elseif op === :GLOBAL_DISTRIBUTE_VECTOR
        if length(IR.args) > 3 && IR.args[4] === :FV
            # FV has elemental dofs
            code = "distribute_solution_FV!("*generate_from_IR_julia(IR.args[2], IRtypes) * ", "* generate_from_IR_julia(IR.args[3], IRtypes) * ", mesh.nel_owned, dofs_per_node, config, buffers);"
        else
            # FE has nodal dofs
            code = "distribute_solution!("*generate_from_IR_julia(IR.args[2], IRtypes) * ", "* generate_from_IR_julia(IR.args[3], IRtypes) * ", nnodes_partition, dofs_per_node, partitioned_order, partitioned_sizes, config, buffers);"
        end
        
    elseif op === :GATHER_VARS
        # place variable arrays in global vector
        if length(IR.args) < 3
            code = generate_from_IR_julia(IR.args[2], IRtypes) * " = get_var_vals(var, "* generate_from_IR_julia(IR.args[2], IRtypes) * ", );";
        else
            code = generate_from_IR_julia(IR.args[2], IRtypes) * 
    ...
    elseif op === :BDRY_TO_VECTOR
        ...
        if length(IR.args) < 3
            code = "copy_bdry_vals_to_vector(var, "* generate_from_IR_julia(IR.args[2], IRtypes) *", mesh, dofs_per_node, prob);";
        else
            code = "copy_bdry_vals_to_vector("* generate_from_IR_julia(IR.args[3], IRtypes) *", "* 
    ...
    elseif op === :BDRY_TO_VAR
        ...
        if length(IR.args) < 4
            code = "copy_bdry_vals_to_variables(var, "* generate_from_IR_julia(IR.args[2], IRtypes) *
                            ", mesh, dofs_per_node, prob, "* generate_from_IR_julia(IR.args[3], IRtypes) *");";
        else
            code = "copy_bdry_vals_to_variables("* generate_from_IR_julia(IR.args[4], IRtypes) *", "* 
    ...
    elseif op === :SCATTER_VARS
        ...
        if length(IR.args) < 3
            code = "place_vector_in_vars(var, "* generate_from_IR_julia(IR.args[2], IRtypes) *");";
        else
            code = "place_vector_in_vars("* generate_from_IR_julia(IR.args[3], IRtypes) *", "* 
                            generate_from_IR_julia(IR.args[2], IRtypes) *");";
        end
        
    elseif op === :LOCAL2GLOBAL
        # put elemental matrix and vector in global system
        code = generate_from_IR_julia(generate_local_to_global_fem(IR.args[2]), IRtypes);
        
    elseif op === :LOCAL2GLOBAL_VEC
        # put elemental vector in global system
        code = generate_from_IR_julia(generate_local_to_global_fem(IR.args[2], vec_only=true), IRtypes);
