...
namespace ot
{
  ...
  {
#ifdef PROFILING
    enum ODATIMERS:unsigned int{
      TimerfilterTreeIn = 0,
      TimerfilterTreeOut = 1,
      TimerfilterTreeSurrogate = 2,
      TimerfilterOctList = 3,
      TimerdistRemesh = 4,
      TimerDistTreeSort = 5,
      TimerCoalesceSiblings = 6,
      TimerSurrogateDAVecs = 7,
      TimeroutTree = 8,
      TimersurrogateTree = 9,
      Timerswap = 10,
      MAXODATIMERS = 11
    };
    static const char* timerNames[] = {"filterTreeIn","filterTreeOut","filterTreeSurrogate","filterOctList",
                                       "distRemesh","DistTreeSort","CoalesceSiblings","SurrogateDAVecs","outTree",
                                       "surrogateTree","swapTree"};

    static_assert(sizeof(timerNames)/sizeof(char*) == ODATIMERS::MAXODATIMERS,
                  "sizes dont match");
    std::array<std::chrono::high_resolution_clock::time_point,ODATIMERS::MAXODATIMERS> timerArrayStart{};
    std::array<std::chrono::high_resolution_clock::time_point,ODATIMERS::MAXODATIMERS> timerArrayEnd{};
    std::array<double ,ODATIMERS::MAXODATIMERS> timings{};
    timings.fill(0.0);
#endif
    MPI_Comm comm = inTree.m_comm;
#ifdef PROFILING
    START_TIMER(ODATIMERS::TimerfilterTreeIn,comm)
#endif
    const std::vector<TreeNode<T, dim>> &inTreeVec = inTree.getTreePartFiltered();
#ifdef PROFILING
    END_TIMER(ODATIMERS::TimerfilterTreeIn,comm)
    START_TIMER(ODATIMERS::TimerdistRemesh,comm)
#endif
...
#ifdef PROFILING
    END_TIMER(ODATIMERS::TimerdistRemesh,comm)
    START_TIMER(ODATIMERS::TimerfilterOctList,comm)
#endif
    DistTree<T, dim>::filterOctList(inTree.getDomainDecider(), outTreeVec);
#ifdef PROFILING
    END_TIMER(ODATIMERS::TimerfilterOctList,comm)

    START_TIMER(ODATIMERS::TimerDistTreeSort,comm)
#endif
    SFC_Tree<T, dim>::distTreeSort(outTreeVec, loadFlexibility, comm);
#ifdef PROFILING
    END_TIMER(ODATIMERS::TimerDistTreeSort,comm)

    START_TIMER(ODATIMERS::TimerCoalesceSiblings,comm)
#endif
    SFC_Tree<T, dim>::distCoalesceSiblings(outTreeVec, comm);
#ifdef PROFILING
    END_TIMER(ODATIMERS::TimerCoalesceSiblings,comm)
    START_TIMER(ODATIMERS::TimerSurrogateDAVecs,comm)
#endif
    std::vector<TreeNode<T, dim>> surrogateTreeVec =
        SFC_Tree<T, dim>::getSurrogateGrid(
            remeshPartition, inTreeVec, outTreeVec, comm);
#ifdef PROFILING
    END_TIMER(ODATIMERS::TimerSurrogateDAVecs,comm)
    START_TIMER(ODATIMERS::TimeroutTree,comm)
#endif
    DistTree outTree(outTreeVec, comm);
#ifdef PROFILING
    END_TIMER(ODATIMERS::TimeroutTree,comm)
    START_TIMER(ODATIMERS::TimerSurrogateDAVecs,comm)
#endif
    DistTree surrogateTree(surrogateTreeVec, comm, NoCoalesce);
#ifdef PROFILING
    END_TIMER(ODATIMERS::TimerSurrogateDAVecs,comm)
    START_TIMER(ODATIMERS::TimerfilterTreeOut,comm)
#endif
    outTree.filterTree(inTree.getDomainDecider());
#ifdef PROFILING
    END_TIMER(ODATIMERS::TimerfilterTreeOut,comm)
    START_TIMER(ODATIMERS::TimerfilterTreeSurrogate,comm)
#endif
    surrogateTree.filterTree(inTree.getDomainDecider());
#ifdef PROFILING
    END_TIMER(ODATIMERS::TimerfilterTreeSurrogate,comm)
    START_TIMER(ODATIMERS::Timerswap,comm)
#endif
    _outTree = outTree;
    _surrogateTree = surrogateTree;
#ifdef PROFILING
    END_TIMER(ODATIMERS::Timerswap,comm)
    int rank; MPI_Comm_rank(comm,&rank);
    static  int count = 0;
    if(not(rank)){
      for (int i = 0; i < ODATIMERS::MAXODATIMERS; i++) {
        std::cout << "[ODA TIMERS] " << count << " " << std::left << std::setw(20) << timerNames[i] << ":" << timings[i] << " s\n";
      }
      std::cout << "\n\n";
    }
    count++;
