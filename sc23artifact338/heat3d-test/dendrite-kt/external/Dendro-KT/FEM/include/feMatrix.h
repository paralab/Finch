...
protected:
    ...
    public:
        ...
        **/
         ...
         */
        void collectMatrixEntries(Mat * J, ot::MatCompactRows & matRowChunks);

        void addMatrixEntriesToPetsc(Mat * J, ot::MatCompactRows & matRowChunks);
        ...
        * */
         ...
         **/
        virtual bool getAssembledMatrix(Mat *J, MatType mtype,ot::MatCompactRows & matCompactRows);
...
template <typename LeafT, unsigned int dim>
void feMatrix<LeafT, dim>::collectMatrixEntries(Mat * J,ot::MatCompactRows & matRowChunks)
{
  ...
  if (m_oda.isActive())
  {
    ...
    while (!treeLoopIn.isFinished())
    {
      ...
      if (treeLoopIn.isPre() && subtreeInfo.isLeaf())
      {
        ...
        // If the size of matRowChunk increases too much that means we are creating two global matrix duplication.
        // Therefore, if it increases beyond a certain limit dump it to the global matrix
        if(matRowChunks.getNumRows() > matRowChunks.getmaxAllowedRows()){
          addMatrixEntriesToPetsc(J,matRowChunks);
        }
      }
      treeLoopIn.step();
    }
 ...
 * Passing matCompactRows prevents deallocation of the memory within subsequent iterations
 ...
 */
template <typename LeafT, unsigned int dim>
bool feMatrix<LeafT,dim>::getAssembledMatrix(Mat *J, MatType mtype, ot::MatCompactRows & matCompactRows)
{

  preMat();
  matCompactRows.clear();
  collectMatrixEntries(J,matCompactRows);
  addMatrixEntriesToPetsc(J,matCompactRows);
  postMat();
...
/**
 *
 * @brief Collect elemental matrices and feed them to Petsc MatSetValues().
 * Note that here we are only using MatFlushAssembly. If the assembly is fnished
 * the call must be made for MatFinalAssembly
 *
 */
template <typename LeafT, unsigned int dim>
void feMatrix<LeafT,dim>::addMatrixEntriesToPetsc(Mat * J, ot::MatCompactRows & matRowChunks){
  const auto & rows = matRowChunks.getRowIdxs();
  const auto & cols = matRowChunks.getColIdxs();
  const auto & colVals = matRowChunks.getColVals();
  for (int r = 0; r < rows.size(); r++) {
    PetscInt row = rows[r];
    MatSetValues(*J,1,&row,matRowChunks.getChunkSize(),&cols[r* matRowChunks.getChunkSize()],
                 &colVals[r* matRowChunks.getChunkSize()],ADD_VALUES);
  }
