...
#define START_TIMER(X,COMM) {timerArrayStart[X] = std::chrono::high_resolution_clock::now();MPI_Barrier(COMM);}
#define END_TIMER(X,COMM) {MPI_Barrier(COMM);timerArrayEnd[X] = std::chrono::high_resolution_clock::now(); \
                            timings[X] +=  (static_cast<std::chrono::duration<double>>(timerArrayEnd[X] - timerArrayStart[X])).count();}
#endif
#endif
