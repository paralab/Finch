...
struct BTInputData : public TALYFEMLIB::InputData {
  ...
  bool ReadFromFile(const std::string &filename = std::string("config.txt")) {
    ...
    solverOptionsBT = read_solver_options(cfg, "solver_options_bt");
  ...
  bool CheckInputData() {
    ...
    if (mfree) {
      if (solverOptionsBT.vals.count("pc_type") == 1) {
        solverOptionsBT.vals.at("pc_type") = "none";
